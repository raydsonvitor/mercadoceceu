...
def search_products(search_term, search_by):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute(f"SELECT * FROM products WHERE {search_by} LIKE ?", ('%' + search_term + '%',))
...
def get_all_products():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM products")  # Obtenha todos os produtos ou a consulta que você precisa
...
def delete_oncredits_by_customer_id(customer_id):
    conn = create_connection()
    cursor = conn.cursor()
    try:

...
class TrueBuyInterface:
    def __init__(self, root, tela_width):
        ...
        self.yon = False
        self.tp_0 = self.tp_1 = self.tp_2 = self.tp_3 = self.tp_4 = self.tp_5 = self.tp_6 = self.tp_7 = self.tp_8 = self.tp_9 = None
    ...
    def imprimir_cupom(self, items, payments):
        print('Cupom não fiscal')
        for item in items:
            print(f'item: {item['item_id']} produto: {item['product_name']} quant: {item['quantity']} X preço: {item['price']} = total {helper.format_to_float(item['quantity'])*helper.format_to_float(item['price'])}')
    ...
    def validar_dados_do_registro_de_mercadorias(self):
        ...
        try:
            ...
            if cfm:
                backend.insert_product((codbar_inserido, desc_inserida, ncm_inserido, ncm_desc_inserido, marca_inserida, nome_inserido, preco_inserido, quantidade_inserida, origem_inserida, data_venc_inserida))
            else:
                pass
    ...
    def abrir_tp_1_0(self):
        ...
        self.tp_1_valor_entry.insert(0, self.total_restante)
    ...
    def encerrar_finzalização_da_compra(self):
        try:
            ...
            print(self.lista_product_ids)
            print(items)
            print(self.payments)
            feedback = backend.insert_sale_into_tables(items, self.payments)#lancar no database
            if self.customer_id:
                cfm = backend.delete_oncredits_by_customer_id(self.customer_id)
                if cfm:
                    CTkMessagebox(self.root, message=f"Os valores foram descontados da conta do cliente de id: {self.customer_id}", icon='check', title='')
                else:
                    CTkMessagebox(self.root, message='Erro na hora de descontar os valores da conta do cliente', icon='cancel', title='Erro')
            if feedback:
                print('oi')
                yon = self.get_yes_or_not(self.root, 'Imprimir Cupom fiscal?')
                if yon:
                    print('oie')
                    self.imprimir_cupom(items, self.payments)
                    print('oiee')
                    self.reset_root()
                    print('oieee')
            else:
                raise Exception('Erro inesperado e raisado.')
        except Exception as e:
            print(e)
    ...
    def abrir_tp_3(self):
        if not self.tp_3:
            ...
            self.result_tree.column(self.result_tree_columns[2], width=150, anchor=CENTER)
            self.result_tree.column(self.result_tree_columns[3], width=200, anchor=CENTER)
...
# TOPLEVEL 5    >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   CADASTRO DE CLIENTES
    ...
    def finalizar_fiacao(self, customer_id):
        ...
        print(customer_id, items)
        cfm = backend.insert_into_oncredit(customer_id, items)
        if cfm:
            CTkMessagebox(self.tp_6, message=f"Fiação confirmada para o cliente: {self.cliente_selecionado.upper()}!", icon='check', title='Sucesso')
            self.fechar_tp_6()
            self.reset_root()
        else:
    ...
    def validate_tp_7(self):
        ...
        try:
            ...
            if cliente_id:
                ...
                data = backend.get_all_data_from_customer_by_id(cliente_id)
                if yon:    
                    self.get_all_data_to_treeview(cliente_id, data)
            else:
                raise Exception
        except Exception as e:
    ...
    def get_all_data_to_treeview(self, customer_id, data):
        try:
            if data:
                print(data)
                for item in data:
    ...
    def validate_tp_8(self):
        ...
        try:
            ...
            try:
                ...
                if yon_1:
                    self.fechar_caixa(self.entrada_dinheiro, self.entrada_cartao, self.sangria, valor_inserido)
            except Exception as e:   
                print(e) 
    ...
    def imprimir_resumo_fechamento_de_caixa(self, date, dados):
        print('Imprimindo o resumo do fechamento de caixa')
    
    def fechar_caixa(self, entrada_dinheiro, entrada_cartao, sangria, valor_inserido):
        try:
            date = helper.get_date()
            dados = {'entrada_dinheiro': self.entrada_dinheiro, 'entrada_cartao': self.entrada_cartao, 'sangria':sangria, 'caixa restante': valor_inserido}
            ...
            if yon_1:        
                self.imprimir_resumo_fechamento_de_caixa(date, dados)
